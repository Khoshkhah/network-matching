...
once. Measured on a real conflation corpus, that happens on the two slowest edges of five.
...
**How `auto` chooses.** Per call, from the source's shape, before any matching work — **two**
independent pressures, so the choice is 2-D rather than a threshold on one number:

```python
W  = profiled_width(A)      # max live profile keys at any vertex -- kills "profiled"
Mo = merge_pressure(A)      # max concurrently-open merges       -- kills "cell"
...
merge. `Mo >= W` means *every* nested split's branch rejoins, so `cell`'s `pending` product and the
profiled key are both maximally loaded — the only place re-basing wins. `merge_pressure` is **not**
a merge's in-degree: one merge of in-degree 10 is still one open merge.

The refusal gate estimates **the engine actually chosen**. Line `100935` is why that matters — it
...
**The `rebase` variant.** `engine="rebase"` re-bases costs to *"since the last split"*, so the key
is the last split rather than every live ancestor — width 1 on a pure out-tree, where the default
