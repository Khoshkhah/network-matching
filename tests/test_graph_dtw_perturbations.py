...
# --------------------------------------------------------------------------------------
# Segment emission (middle-to-middle) + sliver-free pools
# --------------------------------------------------------------------------------------
def test_segment_mode_reports_middle_to_middle_distance():
    ...
    res = _match("split", emission="segment")
    assert _routes(res) == ["B1", "B2", "B3"]
...
def test_midpoint_is_alias_for_segment():
    # "midpoint" is kept as a deprecated alias -> identical result to "segment"
    a = _match("split", emission="segment", coords_a=lateral_shift(
        get_scenario("split")["coords_a"], 3.0))
    b = _match("split", emission="midpoint", coords_a=lateral_shift(
        get_scenario("split")["coords_a"], 3.0))
    assert _routes(a) == _routes(b)
    assert a["avg_distance"] == pytest.approx(b["avg_distance"])


def test_segment_states_are_never_point_like():
    # segment mode must pair genuine segments: with the default min_pool_gap (step/2) no
    ...
    res = _match("curve", coords_a=noisy, emission="segment", debug=True)
